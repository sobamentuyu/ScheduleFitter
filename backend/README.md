...
このテストでは、Geminiが返す予定候補について、必須キー、各値の型、ISO 8601日時、開始・終了日時の前後関係、および予定作成に必要な項目に応じたステータスの決定を検証します。
...
5.  **構造化データの検証**:
    Gemini から返されたJSONをデコードし、必須キー、各値の型、日時形式を検証します。予定名、開始日時、終了日時が揃っていれば `ready`、いずれかが不足していれば `needs_clarification` として `status` を設定し、`suggestion` フィールドに入れて返します。
...
| フィールド | 型 | 説明 |
| :--- | :--- | :--- |
| `status` | `string` | バックエンドが予定名、開始日時、終了日時から決定する。すべて揃っていれば `ready`、いずれかが不足していれば `needs_clarification`。 |
