		...
		<>
			...
			>
				<div className="flex shrink-0 items-center justify-end p-2">
					...
					>
						<XIcon size={32} color="var(--color-primary-content)" weight="light" />
					</button>
				</div>
						...
						{chat.messages.map((msg, index) => {
							...
							return (
								<div key={msg.id} className="w-full">
									{isNewDay && (
										<Text as="p" size="xs" color="muted" className="my-3 text-center">
				...
				<div className="flex shrink-0 items-end gap-2 border-t border-base-300 p-2">
					...
					>
						...
						<div className={`grid min-w-0 ${isMultiline ? 'w-full' : 'flex-1'}`}>
							<textarea
								aria-label="メッセージ"
								rows={1}
						...
						>
							<button
								...
								disabled={!voice.browserSupportsSpeechRecognition || !voice.isMicrophoneAvailable}
								onClick={voice.toggleListening}
								className="btn btn-circle btn-ghost btn-xs"
							>
								{voice.listening ? (
									<StopIcon size={18} weight="fill" color="var(--color-primary-content)" />
								) : (
									<MicrophoneIcon size={18} weight="light" color="var(--color-primary-content)" />
								)}
							</button>
							<button
								type="button"
								...
								className="btn btn-circle btn-ghost btn-xs"
							>
								<ImageIcon size={18} weight="light" color="var(--color-primary-content)" />
							</button>
						</div>
					</div>
					<button
						...
						disabled={chat.message.trim() === '' || chat.isSending || voice.listening}
						className={`btn btn-circle btn-lg ${
							voice.listening ? 'bg-gray-400' : 'bg-[var(--color-chat)]'
						}`}
					>
						<ArrowUpIcon size={30} color="var(--color-primary-content)" weight="light" />
					</button>
				</div>
			</aside>
			...
			{!chat.isChatOpen && (
				...
				>
					<PlusIcon size={32} color="var(--color-primary-content)" weight="light" />
				</button>
			)}
		</>
	);
